...
export const verifyCode = async (req, res) => {
    try {
        ...
        if (!admin.verificationCode) {
            return res.status(400).json({ status: "error", message: "Verification code not found for this user." });
        }
        ...
        res.cookie('token', token, { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 3 * 60 * 60 * 1000 });
        res.status(200).json({ status: "success", message: "Verification successful, login successful" });
    } catch (error) {
